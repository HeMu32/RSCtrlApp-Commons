...
7. 音视频对齐职责边界：
  - UniAVFrame 仅作为音视频帧与元数据的统一承载对象，不在本层实现 A/V 对齐。
  - 不在本层做重采样、重切帧、编码器输入对齐（例如 AAC 常见帧大小）等处理。
  - 对于 BMD 采集链路与 FFmpeg 编解码链路在帧边界/节奏上的差异，
    由回放端或编码端实现自行维护时间线、缓存与切片策略。
...
### BMDSDK 视频后端：`originalMemory().data` 为 `nullptr`

`MemoryView originalMemory()` 对 BMDSDK 视频帧返回 `{nullptr, N}`（`data` 为空，`sizeBytes` 为帧字节大小估算值）。

原因：`MemoryView` 是非拥有的裸指针结构，不具备生命周期绑定能力。BMD SDK 不承诺
`EndAccess()` 调用后字节指针仍然有效，因此存储该指针是不安全的行为。
对于 FFmpeg、Qt、OpenCV，`data` 指向的内存由 `m_nativeOwner` 的生命周期保护，BMDSDK 视频路径无法提供同等保证。

补充：BMDSDK 音频包路径（`IDeckLinkAudioInputPacket`）中，`GetBytes()` 返回的包内存可在包对象生命周期内访问，
UniAVFrame 通过 AddRef/Release 接管生命周期后，`originalMemory().data` 可安全暴露为只读视图。
