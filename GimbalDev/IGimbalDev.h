...
public:
    ...
    /**
     * @brief           摇杆相对增量移动
     * @param   nX      水平方向归一化输入（映射至偏航轴）
     * @param   nY      垂直方向归一化输入（映射至俯仰轴）
     *
     * @note    推荐的统一数值区间为 `[-15000, 15000]`。
     *          `0` 表示该轴无输入，正负号表示相反方向。
     *          若调用方传入越界值，实现层应裁剪到该区间内。
     *          正数表示镜头指向右/上，负数表示镜头指向左/上。
     *          此处向右指: 俯视摄像机, 摄像机顺时针转动
     *          此处向上指: 侧视且摄像机指向左侧, 摄像机顺时针转动   
     *          这一约定取自 DJI R SDK 当前用的 joystick 数值语义。
